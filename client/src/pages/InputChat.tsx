        ...
        (error) => {
          ...
          switch(error.code) {
            case error.PERMISSION_DENIED:
              ...
              errorMessage += "Please check your browser settings and allow access to your location.";
              console.warn("Make sure you've allowed location access in your browser settings.");
              break;
              
            case error.POSITION_UNAVAILABLE:
              errorMessage += "Location information is unavailable. ";
              errorMessage += "Please check your internet connection and ensure GPS is enabled.";
              break;
              
            case error.TIMEOUT:
              errorMessage += "The request to get your location timed out. ";
              errorMessage += "Please try again in an area with better GPS reception.";
              break;
        ...
        {locationError ? (
          <div className="map-error">
            ...
            <div className="error-instructions">
              ...
              <ol>
                <li>Check that your browser has permission to access your location</li>
                <li>Make sure your GPS is enabled</li>
                <li>Refresh the page and accept the permission request</li>
                <li>Check your browser's site settings if the permission prompt doesn't appear</li>
              </ol>
            </div>
          </div>
        ) : null}
        {nextEvent ? (
          <>
            <div className="event-details">
              <h4><FiCalendar /> {nextEvent.summary || 'Appointment'}</h4>
              {nextEvent.address && (
                <p><FiMapPin /> {nextEvent.address}</p>
              )}
              {nextEvent.startTime && (
                <p><FiClock /> {new Date(nextEvent.startTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                  {nextEvent.endTime && ` - ${new Date(nextEvent.endTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}`}
                </p>
              )}
            </div>
